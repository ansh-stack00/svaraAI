...
import { Bot, Phone, Clock, TrendingUp, Plus, ArrowUpRight } from "lucide-react";
...
export default async function DashboardPage() {
    ...
    const avgDuration = callCount ? Math.floor(totalDuration / callCount) : 0;

    const successRate = callCount ? Math.floor(
            (calls.filter((c) => c.status === "completed").length / callCount) * 100  ) : 0
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <Card className="border-slate-700 bg-slate-800/50 backdrop-blur-sm shadow-2xl">
          <CardHeader className="border-b border-slate-700 pb-6">
            <CardTitle className="text-2xl text-white">Recent Calls</CardTitle>
          </CardHeader>
            ...
            {recentCalls.length > 0 ? (
              <div className="space-y-3">
                {recentCalls.map((call) => (
                  <div
                    key={call.id}
                    className="flex justify-between items-center p-4 bg-slate-700/40 rounded-lg hover:bg-slate-700/60 transition-colors border border-slate-600/50"
                  >
